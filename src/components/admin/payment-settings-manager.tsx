...
function EnvironmentForm({ settings, webhookUrl }: { settings: MercadoPagoAdminSettings; webhookUrl: string }) {
  ...
  return <Card className={settings.active ? "border-primary shadow-md" : ""}>
    ...
    <CardContent className="space-y-5">
      <div className="space-y-2"><Label htmlFor={`public-${settings.environment}`}>Public Key</Label><Input id={`public-${settings.environment}`} value={draft.publicKey} onChange={(event) => setDraft({ ...draft, publicKey: event.target.value })} placeholder={settings.environment === "TEST" ? "TEST-..." : "APP_USR-..."} autoComplete="off" /></div>
      <div className="space-y-2"><Label htmlFor={`token-${settings.environment}`}>Access Token</Label><div className="flex gap-2"><Input id={`token-${settings.environment}`} type={showSecrets ? "text" : "password"} value={draft.accessToken} onChange={(event) => setDraft({ ...draft, accessToken: event.target.value })} placeholder={settings.access_token_configured ? "Configurado — deixe vazio para manter" : settings.environment === "TEST" ? "TEST-..." : "APP_USR-..."} autoComplete="new-password" /><Button type="button" size="icon" variant="outline" onClick={() => setShowSecrets(!showSecrets)} aria-label={showSecrets ? "Ocultar segredos" : "Mostrar segredos"}>{showSecrets ? <EyeOff /> : <Eye />}</Button></div></div>
      ...
      <div className="rounded-xl border bg-muted/50 p-3"><p className="text-xs font-bold uppercase text-muted-foreground">URL do webhook · evento Pagamentos</p><div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 break-all text-xs">{webhookUrl}</code><Button type="button" size="icon-sm" variant="outline" onClick={() => navigator.clipboard.writeText(webhookUrl).then(() => toast.success("URL copiada."))} aria-label="Copiar URL"><Copy /></Button></div><p className="mt-2 text-xs text-muted-foreground">Integração: Checkout Transparente · API de Pagamentos.</p></div>
